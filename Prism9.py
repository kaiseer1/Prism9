#!/usr/bin/env python3
"""
Prism9: Enhanced AI-driven Framework for Proactive Cybersecurity Defense
With Real-time Streaming Analytics, Advanced RZA/FPP, CSV Training, and Bug Fixes

Author: Based on research by Basil Abdullah Alzahrani
Enhanced with streaming analytics, CSV training capabilities, and bug fixes
"""
...
main_handler = logging.FileHandler(log_dir / f'Prism9_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
training_handler = logging.FileHandler(log_dir / 'ml_training.log')
...
class SecuritySignal:
    ...
    def to_dict(self):
        """Convert to dictionary for logging with proper error handling"""
        try:
            return {
            ...
            logging.error(f"Error converting SecuritySignal to dict: {e}")
            return {
                'timestamp': self.timestamp,
                'source_module': self.source_module,
                'signal_type': self.signal_type,
    ...
    def _convert_numpy_types(self, obj, seen=None):
        """Convert numpy types to Python native types for JSON serialization with circular reference protection"""
        if seen is None:
            seen = set()
...
class CSVTrainingEngine:
    """CSV-based training engine for Prism9 ML models"""
    
    def __init__(self):
        self.logger = logging.getLogger("CSVTraining")
        self.logger.addHandler(training_handler)
        
        # Model storage
        self.models_dir = Path("Prism9_models")
        ...
        # CSV file paths - Fixed paths for the specific datasets
        self.csv_files = [
            "Thursday-WorkingHours-Morning-WebAttacks.pcap_ISCX.csv",
            "Thursday-WorkingHours-Afternoon-Infilteration.pcap_ISCX.csv"
        ]
        
        self.logger.info("CSV Training Engine initialized")
    
    def load_and_preprocess_csv(self, csv_path: str) -> pd.DataFrame:
        """Load and preprocess CSV data with comprehensive error handling"""
        try:
            ...
            # Handle common CICIDS2017 format
            if 'label' in df.columns:
                # Convert labels to binary (BENIGN vs ATTACK)
                df['is_attack'] = df['label'].apply(lambda x: 0 if str(x).upper() == 'BENIGN' else 1)
            elif ' label' in df.columns:
                df['is_attack'] = df[' label'].apply(lambda x: 0 if str(x).strip().upper() == 'BENIGN' else 1)
            else:
                # Create synthetic labels based on anomaly detection
                self.logger.warning("No label column found, creating synthetic labels")
                df['is_attack'] = 0
            
            # Feature engineering for network traffic
            if 'flow_duration' in df.columns:
                df['log_flow_duration'] = np.log1p(df['flow_duration'].fillna(0))
            
            if 'total_fwd_packets' in df.columns and 'total_backward_packets' in df.columns:
                df['packet_ratio'] = (df['total_fwd_packets'] / 
                                    (df['total_backward_packets'] + 1)).fillna(0)
    ...
    def extract_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        try:
            ...
            target_cols = ['is_attack', 'label', ' label']
            feature_cols = [col for col in numeric_cols if col not in target_cols]
            
            if not feature_cols:
                self.logger.error("No numeric features found in dataset")
                return np.array([]), np.array([])
            
            # Extract features and labels
            X = df[feature_cols].values
            y = df['is_attack'].values if 'is_attack' in df.columns else np.zeros(len(df))
            
            # Store feature names for later use
            self.feature_names = feature_cols
            
            self.logger.info(f"Extracted {X.shape[1]} features from {X.shape[0]} samples")
            self.logger.info(f"Attack ratio: {np.mean(y):.3f}")
    ...
    def train_models(self) -> bool:
        ...
        try:
            ...
            for csv_file in self.csv_files:
                df = self.load_and_preprocess_csv(csv_file)
                if not df.empty:
            ...
            self.logger.info(f"Combined training data: {X_combined.shape}")
            ...
            if len(benign_samples) > 10:  # Need minimum samples
                ...
                if np.sum(y_test) > 0:  # If there are attacks in test set
                    self.model_metrics['anomaly_precision'] = precision_score(y_test, anomaly_binary, zero_division=0)
                    self.model_metrics['anomaly_recall'] = recall_score(y_test, anomaly_binary, zero_division=0)
            ...
            # Evaluate classifier
            classifier_accuracy = self.attack_classifier.score(X_test_scaled, y_test)
            self.model_metrics['classifier_accuracy'] = classifier_accuracy
            ...
            self.logger.info(f"Anomaly Detection - Precision: {self.model_metrics['anomaly_precision']:.3f}, "
                           f"Recall: {self.model_metrics['anomaly_recall']:.3f}")
            self.logger.info(f"Classifier Accuracy: {self.model_metrics['classifier_accuracy']:.3f}")
    ...
    def save_models(self):
        ...
        try:
            ...
            joblib.dump(self.attack_classifier, self.models_dir / 'attack_classifier.pkl')
    ...
    def load_models(self) -> bool:
        ...
        try:
            if not (self.models_dir / 'anomaly_detector.pkl').exists():
                self.logger.warning("No saved models found")
                return False
            
            self.anomaly_detector = joblib.load(self.models_dir / 'anomaly_detector.pkl')
            self.attack_classifier = joblib.load(self.models_dir / 'attack_classifier.pkl')
    ...
    def predict_anomaly(self, features: np.ndarray) -> Tuple[float, float]:
        ...
        try:
            if hasattr(self.anomaly_detector, 'decision_function'):
                ...
                attack_prob = self.attack_classifier.predict_proba(features_scaled)[0][1]
                return float(anomaly_score), float(attack_prob)
            else:
                return 0.0, 0.0
        except Exception as e:
...
class StreamingAnalytics:
    """Real-time streaming analytics engine for continuous monitoring - Bug Fixed"""
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.lock = threading.RLock()  # Fixed: Use RLock to prevent deadlocks
        ...
        # Fixed: Add shutdown flag and proper thread management
        self.running = True
        self.analytics_thread = None
        self._start_analytics_thread()
    
    def _start_analytics_thread(self):
        """Start analytics thread with proper error handling"""
        try:
            self.analytics_thread = threading.Thread(target=self._analyze_stream, daemon=True)
            self.analytics_thread.start()
            self.logger.info("Analytics thread started")
        except Exception as e:
            self.logger.error(f"Error starting analytics thread: {e}")
    
    def add_event(self, signal: SecuritySignal, response_time: float):
        """Add new event to streaming analytics with error handling"""
        try:
            with self.lock:
                ...
                if 'anomaly_score' in signal.data:
                    self.anomaly_scores.append(float(signal.data['anomaly_score']))
                
                if 'trust_score' in signal.data:
                    self.trust_scores.append(float(signal.data['trust_score']))
                
                if 'patterns' in signal.data:
                    for pattern in signal.data['patterns']:
                        self.pattern_frequencies[pattern] += 1
                        
                # Fixed: Limit pattern frequencies to prevent memory leak
                if len(self.pattern_frequencies) > 1000:
                    ...
                    for pattern, _ in sorted_patterns[:100]:  # Remove bottom 100
                        del self.pattern_frequencies[pattern]
                        
        except Exception as e:
            self.logger.error(f"Error adding event to analytics: {e}")
    
    def _calculate_entropy(self) -> float:
        """Calculate Shannon entropy of pattern distribution with error handling"""
        try:
    ...
    def _detect_burst(self) -> bool:
        """Detect sudden burst in threat levels with error handling"""
        try:
            ...
            self.logger.error(f"Error detecting burst: {e}")
            return False
    
    def _analyze_stream(self):
        """Continuous stream analysis thread with improved error handling"""
        while self.running:
            try:
                with self.lock:
                    if len(self.threat_stream) > 0:
                        self.stats['avg_threat_level'] = float(np.mean(self.threat_stream))
                    
                    if len(self.response_times) > 0:
                        self.stats['avg_response_time'] = float(np.mean(self.response_times))
                    
                    if len(self.anomaly_scores) > 1:
                        # Calculate trend using linear regression
                        x = np.arange(len(self.anomaly_scores))
                        self.stats['anomaly_trend'] = float(np.polyfit(x, self.anomaly_scores, 1)[0])
                    
                    if len(self.trust_scores) > 1:
                        x = np.arange(len(self.trust_scores))
                        self.stats['trust_trend'] = float(np.polyfit(x, self.trust_scores, 1)[0])
                    
                    self.stats['pattern_entropy'] = self._calculate_entropy()
                    self.stats['burst_detected'] = self._detect_burst()
                    
                    # Log streaming analytics summary (reduced frequency)
                    if random.random() < 0.05:  # Log 5% of the time to reduce spam
                        self.logger.debug(f"Stream Stats: {json.dumps(self.stats, indent=2)}")
                
                time.sleep(2)  # Analyze every 2 seconds to reduce CPU usage
                
            except Exception as e:
                self.logger.error(f"Error in analytics stream: {e}")
    ...
    def get_recommendations(self) -> Dict:
        ...
        try:
            ...
            return {'timestamp': time.time(), 'stats': {}, 'recommendations': []}
    
    def stop(self):
        """Stop analytics thread safely"""
        self.running = False
        if self.analytics_thread and self.analytics_thread.is_alive():
            ...
            if self.analytics_thread.is_alive():
                self.logger.warning("Analytics thread did not stop gracefully")

class SecureSignalBus:
    """Enhanced signal bus with streaming analytics integration - Bug Fixed"""
    
    def __init__(self, analytics: StreamingAnalytics):
        self.subscribers = {}
        self.signal_history = deque(maxlen=1000)  # Fixed: Limit history size to prevent memory leak
        self.lock = threading.RLock()  # Fixed: Use RLock
    ...
    def subscribe(self, module_name: str, callback):
        """Register module to receive signals with error handling"""
        try:
            with self.lock:
            ...
            self.logger.error(f"Error subscribing module {module_name}: {e}")
    ...
    def publish(self, signal: SecuritySignal):
        ...
        try:
            with self.lock:
                ...
                signal_data = f"{signal.timestamp}{signal.source_module}{str(signal.data)}"
                signal_hash = hashlib.sha256(signal_data.encode()).hexdigest()[:16]
                
                self.signal_history.append((signal, signal_hash))
                
                # Log key signal details (with error handling)
                try:
                    signal_dict = signal.to_dict()
                    self.logger.debug(f"Signal Published: {json.dumps(signal_dict, indent=2)}")
                except Exception as log_error:
                    self.logger.warning(f"Error logging signal details: {log_error}")
...
class RZA:
    """
    Enhanced Reverse Zero-day Algorithm with ML integration and bug fixes
    """
    
    def __init__(self, signal_bus: SecureSignalBus, csv_engine: CSVTrainingEngine = None):
        ...
        self.anomaly_history = deque(maxlen=1000)  # Fixed: Limit size
        self.pattern_cache = {}
        self._cache_lock = threading.Lock()  # Fixed: Add cache lock
    ...
    def _extract_ml_features(self, code_input: str) -> Dict[str, float]:
        ...
        try:
            ...
            features['length_anomaly'] = abs(len(code_input) - avg_length) / max(avg_length, 1)
            ...
            return {'entropy': 0, 'length_anomaly': 0, 'char_distribution': 0, 
                   'pattern_density': 0, 'temporal_correlation': 0}
    ...
    def _get_contextual_multiplier(self) -> float:
        ...
        try:
            ...
            self.logger.error(f"Error getting contextual multiplier: {e}")
            return self.contextual_thresholds['default']
    
    def analyze_code_patterns(self, code_input: str) -> Tuple[float, List[str]]:
        """Enhanced pattern analysis with ML features and caching"""
        anomaly_score = 0.0
        detected_patterns = []
        pattern_details = []
        ...
        try:
                ...
                for pattern in pattern_data.get('patterns', []):
                    try:
                        ...
                        for match in matches:
                            ...
                            weight *= (1 + 0.1 * len(match.group()) / max(len(code_input), 1))
            ...
            if self.csv_engine and hasattr(self.csv_engine, 'predict_anomaly'):
                try:
                    # Create feature vector for ML prediction
                    ml_feature_vector = np.array([
                        ml_features['entropy'],
                        ml_features['length_anomaly'],
                        ml_features['char_distribution'],
                        ml_features['pattern_density'],
                    ...
                    if len(ml_feature_vector) < 20:  # Assume 20 features expected
                        ml_feature_vector = np.pad(ml_feature_vector, (0, 20 - len(ml_feature_vector)))
                    
                    ml_anomaly_score, ml_attack_prob = self.csv_engine.predict_anomaly(ml_feature_vector[:20])
                    
                    # Combine traditional and ML scores
                    anomaly_score = 0.7 * anomaly_score + 0.3 * abs(ml_anomaly_score) * 10
                    
                    self.logger.debug(f"ML integration: anomaly={ml_anomaly_score:.3f}, attack_prob={ml_attack_prob:.3f}")
                    
                except Exception as ml_error:
                    self.logger.debug(f"ML prediction failed: {ml_error}")
            ...
            with self._cache_lock:
                ...
                if len(self.pattern_cache) > 1000:
                    oldest_entries = sorted(self.pattern_cache.items(), 
    ...
    def _calculate_behavioral_anomaly(self, input_data: str) -> float:
        """Enhanced behavioral anomaly detection with error handling"""
        try:
            features = [
                len(input_data),
                input_data.count(';'),
            ...
            self.logger.error(f"Error calculating behavioral anomaly: {e}")
            return 0.0
    
    def _update_baseline(self, new_score: float):
        """Update baseline with outlier detection and error handling"""
        try:
    ...
    def process_input(self, input_data: str, context: Dict) -> SecuritySignal:
        """Main RZA processing with comprehensive logging and error handling"""
        start_time = time.time()
        ...
        try:
            ...
            self.logger.error(f"Error in RZA processing: {e}")
            # Return safe default signal
            return SecuritySignal(
                timestamp=time.time(),
                source_module="RZA",
                signal_type="vulnerability_prediction",
                data={"error": str(e), "anomaly_score": 0.0, "patterns": []},
                confidence=0.0,
                threat_level=ThreatLevel.LOW
            )
    
    def receive_signal(self, signal: SecuritySignal):
        """Enhanced signal processing with learning and error handling"""
        try:
            ...
            self.logger.error(f"Error processing received signal: {e}")
    
    def _update_patterns(self, attack_data: Dict):
        """Update patterns based on confirmed attacks with error handling"""
        try:
...
class TVM:
    """
    Enhanced Zero-Trust Verification Module with adaptive trust scoring and bug fixes
    """
    ...
    def __init__(self, signal_bus: SecureSignalBus):
        ...
        # Trust history for trend analysis - Fixed: Limit size per user
    ...
    def _calculate_trust_trend(self, user_id: str) -> float:
        """Calculate trust trend for a user with error handling"""
        try:
            ...
            self.logger.error(f"Error calculating trust trend: {e}")
            return 0.0
    
    def _evaluate_location_risk(self, location: str) -> float:
        """Enhanced location risk evaluation with error handling"""
        try:
            ...
            for category in ['blocked_locations', 'suspicious_locations', 'trusted_locations']:
                if location in policies[category]:
                    if isinstance(policies[category], dict):
                        return float(policies[category][location])
                    else:
            ...
            self.logger.error(f"Error evaluating location risk: {e}")
            return 0.5
    
    def _evaluate_device_trust(self, device_id: str) -> float:
        """Enhanced device trust evaluation with error handling"""
        try:
    ...
    def _evaluate_behavioral_score(self, context: UserContext) -> float:
        """Enhanced behavioral evaluation with pattern analysis and error handling"""
        try:
            current_hour = datetime.now().hour
            base_score = 1.0
            ...
            self.logger.error(f"Error evaluating behavioral score: {e}")
            return 0.5
    
    def evaluate_trust(self, user_context: UserContext) -> float:
        """Calculate comprehensive trust score with logging and error handling"""
        trust_components = {}
        ...
        try:
            ...
            return 0.0  # Fail securely
    
    def process_access_request(self, user_context: UserContext, resource: str) -> SecuritySignal:
        """Process access request with enhanced decision logging and error handling"""
        start_time = time.time()
        ...
        try:
            ...
            return SecuritySignal(
                ...
                source_module="TVM",
                signal_type="trust_evaluation",
                data={"error": str(e), "trust_score": 0.0},
                confidence=0.0,
                threat_level=ThreatLevel.CRITICAL
            )
    
    def receive_signal(self, signal: SecuritySignal):
        """Process signals from other modules with error handling"""
        try:
...
class FPP:
    """
    Enhanced False Positive Protocol with sophisticated deception strategies and bug fixes
    """
    ...
    def __init__(self, signal_bus: SecureSignalBus):
        ...
        # Attacker profiling - Fixed: Limit size to prevent memory leak
        self.attacker_profiles = {}
        self._profile_lock = threading.Lock()
    
    def _profile_attacker(self, context: Dict) -> str:
        """Profile attacker based on behavior patterns with error handling"""
        try:
            ...
            self.logger.error(f"Error profiling attacker: {e}")
            return 'automated'
    ...
    def _select_deception_type(self, attacker_profile: str, context: Dict) -> str:
        ...
        try:
            ...
            self.logger.error(f"Error selecting deception type: {e}")
            return 'tar_pit'
    
    def calculate_deception_score(self, context: Dict) -> float:
        """Enhanced deception scoring with behavioral analysis and error handling"""
        try:
            base_score = 0.3
            ...
            self.logger.error(f"Error calculating deception score: {e}")
            return 0.0
    
    def deploy_deception(self, threat_context: Dict) -> Dict:
        """Deploy sophisticated deception mechanism with error handling"""
        try:
            ...
            # Update attacker profile with thread safety
            with self._profile_lock:
                ...
                # Clean old profiles to prevent memory leak
                if len(self.attacker_profiles) > 1000:
                    old_profiles = [(k, v) for k, v in self.attacker_profiles.items() 
                                  if time.time() - v['first_seen'] > 86400]  # 24 hours old
                    for old_id, _ in old_profiles[:100]:  # Remove oldest 100
                        del self.attacker_profiles[old_id]
            ...
            self.logger.error(f"Error deploying deception: {e}")
            return {}
    
    def process_threat_signal(self, rza_signal: SecuritySignal, tvm_signal: SecuritySignal) -> SecuritySignal:
        """Process combined signals with sophisticated deception decision and error handling"""
        start_time = time.time()
        ...
        try:
            ...
            if hasattr(self.signal_bus, 'analytics') and self.signal_bus.analytics.stats.get('burst_detected', False):
            ...
            self.logger.error(f"Error processing threat signal: {e}")
            # Return safe default signal
            return SecuritySignal(
                timestamp=time.time(),
                source_module="FPP",
                signal_type="deception_decision",
                data={"error": str(e), "deception_score": 0.0, "response": "MONITOR"},
                confidence=0.0,
                threat_level=ThreatLevel.LOW
            )
    
    def _count_recent_attempts(self, user_id: str) -> int:
        """Count recent attempts from a user with error handling"""
        try:
            ...
            self.logger.error(f"Error counting recent attempts: {e}")
            return 1
    ...
    def _mark_deception_success(self, attacker_id: str):
        """Mark deception as successful and update metrics with error handling"""
        try:
            success_count = 0
            ...
            self.logger.error(f"Error marking deception success: {e}")

class Prism9Framework:
    """
    Enhanced Prism9 Framework with streaming analytics, CSV training, and comprehensive logging
    """
    
    def __init__(self):
        # Initialize CSV training engine first
        self.csv_engine = CSVTrainingEngine()
        
        # Try to load existing models or train new ones
        if not self.csv_engine.load_models():
            self.logger = logging.getLogger("Prism9")
            self.logger.info("No existing models found. Training new models...")
            if self.csv_engine.train_models():
                self.logger.info("Model training completed successfully")
            else:
                self.logger.warning("Model training failed, continuing with rule-based detection")
        ...
        self.rza = RZA(self.signal_bus, self.csv_engine)
        ...
        self.logger.info("=" * 80)
        self.logger.info("Prism9 FRAMEWORK INITIALIZED - ENHANCED VERSION")
        self.logger.info(f"Version: 2.1 Enhanced with CSV Training")
        ...
        self.logger.info(f"ML Models: {'Loaded' if hasattr(self.csv_engine, 'anomaly_detector') else 'Training Required'}")
        self.logger.info(f"CSV Files: {', '.join(self.csv_engine.csv_files)}")
        self.logger.info("=" * 80)
    
    def process_security_event(self, input_data: str, user_context: UserContext, resource: str) -> ResponseAction:
        """
        Enhanced main processing with comprehensive logging, analytics, and error handling
        """
        start_time = time.time()
        ...
        try:
            ...
            delta_t = rza_signal.data.get("anomaly_score", 0)
            mu_k_sigma = rza_signal.data.get("threshold", 3.0)
            trust_score = tvm_signal.data.get("trust_score", 1.0)
            ...
            if anomaly_detected and trust_low:
                ...
                f_x = fpp_signal.data.get("deception_score", 0)
            ...
            return ResponseAction.BLOCK  # Fail securely
    
    def retrain_models(self) -> bool:
        """Retrain ML models with new data"""
        try:
            self.logger.info("Retraining ML models...")
            success = self.csv_engine.train_models()
            if success:
    ...
    def get_statistics(self) -> Dict:
        """Get comprehensive framework statistics with error handling"""
        try:
            ...
            stats = {
                ...
                "ml_model_metrics": self.csv_engine.model_metrics
            }
    ...
    def shutdown(self):
        ...
        try:
            self.logger.info("Shutting down Prism9 Framework...")
            self.analytics.stop()
            ...
            for key, value in stats.items():
                if key not in ['streaming_analytics', 'ml_model_metrics']:
                    self.logger.info(f"  {key}: {value}")
        except Exception as e:
            self.logger.error(f"Error during shutdown: {e}")

# Keep the existing helper functions with bug fixes
...
def main():
    """Enhanced demonstration of Prism9 Framework with CSV training and comprehensive logging"""
    print("=" * 80)
    print("Prsim9 CYBERSECURITY FRAMEWORK - ENHANCED DEMO")
    print("Version 2.1 with CSV Training and Bug Fixes")
    print("=" * 80)
    print()
    
    # Initialize framework
    Prism9 = Prism9Framework()
    ...
    try:
        ...
        for i in range(num_scenarios):
            ...
            # Process event
            response = Prism9.process_security_event(attack_input, user, resource)
        ...
        ml_metrics = stats.get('ml_model_metrics', {})
        print(f"  Anomaly Detection Precision: {ml_metrics.get('anomaly_precision', 0):.3f}")
        print(f"  Anomaly Detection Recall: {ml_metrics.get('anomaly_recall', 0):.3f}")
        print(f"  Attack Classifier Accuracy: {ml_metrics.get('classifier_accuracy', 0):.3f}")
        print(f"  Last Training: {ml_metrics.get('last_training', 'Never')}")
        
        print("\nStreaming Analytics:")
        streaming_stats = stats.get('streaming_analytics', {})
        print(f"  Average Threat Level: {streaming_stats.get('avg_threat_level', 0):.2f}")
        print(f"  Anomaly Trend: {streaming_stats.get('anomaly_trend', 0):+.3f}")
        print(f"  Trust Trend: {streaming_stats.get('trust_trend', 0):+.3f}")
        print(f"  Pattern Entropy: {streaming_stats.get('pattern_entropy', 0):.2f}")
        print(f"  Burst Detected: {streaming_stats.get('burst_detected', False)}")
        
        print(f"\nEnhanced Features Demonstrated:")
        print(f"  ✓ CSV-based ML training with CICIDS2017 format support")
        print(f"  ✓ Real-time streaming analytics with trend detection")
        print(f"  ✓ Bug fixes: memory leak prevention, thread safety, error handling")
        ...
        print(f"\nSupported CSV Files:")
        csv_files = ["Thursday-WorkingHours-Morning-WebAttacks.pcap_ISCX.csv",
                    "Thursday-WorkingHours-Afternoon-Infilteration.pcap_ISCX.csv"]
        for csv_file in csv_files:
            status = "✓ Found" if Path(csv_file).exists() else "✗ Not Found"
            print(f"  {status} {csv_file}")
        
        print(f"\nLog Files Created:")
        log_files = list(Path("Prism9_logs").glob("*.log"))
        ...
        print(f"\nModel Files:")
        model_files = list(Path("Prism9_models").glob("*"))
        if model_files:
            for model_file in model_files:
                print(f"  - {model_file.name}")
        else:
            print(f"  No model files found (CSV files may be missing)")
        
        # Test model retraining if CSV files are available
        csv_available = any(Path(csv).exists() for csv in csv_files)
        if csv_available:
            print(f"\n{'='*60}")
            print("TESTING MODEL RETRAINING")
            print(f"{'='*60}")
            
            print("Retraining models with available CSV data...")
            retrain_success = Prism9.retrain_models()
            
            if retrain_success:
                print("✓ Model retraining completed successfully")
                updated_stats = Prism9.get_statistics()
                updated_ml_metrics = updated_stats.get('ml_model_metrics', {})
                print(f"Updated Metrics:")
                print(f"  Anomaly Detection Precision: {updated_ml_metrics.get('anomaly_precision', 0):.3f}")
                print(f"  Anomaly Detection Recall: {updated_ml_metrics.get('anomaly_recall', 0):.3f}")
                print(f"  Attack Classifier Accuracy: {updated_ml_metrics.get('classifier_accuracy', 0):.3f}")
            else:
                print("✗ Model retraining failed")
        else:
            print(f"\nNote: Place the following CSV files in the current directory for ML training:")
            for csv_file in csv_files:
                print(f"  - {csv_file}")
        ...
        # Cleanup
        Prism9.shutdown()
        print("\nDemo completed. Check 'Prism9_logs' directory for detailed logs.")
        print("Check 'Prism9_models' directory for trained ML models.")
